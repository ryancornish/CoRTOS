...
alignas(CORTOS_STACK_ALIGN) static constinit std::array<std::byte, STACK_BYTES> timed1_stack{};
alignas(CORTOS_STACK_ALIGN) static constinit std::array<std::byte, STACK_BYTES> timed2_stack{};
alignas(CORTOS_STACK_ALIGN) static constinit std::array<std::byte, STACK_BYTES> blocking_stack{};
alignas(CORTOS_STACK_ALIGN) static constinit std::array<std::byte, STACK_BYTES> monitor_stack{};
...
static void timed_worker(char const* name)
{
   ...
   while (true) {
      ...
      auto const deadline = cortos::Scheduler::tick_now() + 5;
      if (mutex.try_lock_until(deadline)) {
         now = cortos::Scheduler::tick_now().value();
         ++shared_counter;
...
int main()
{
   ...
   cortos::Thread timed_worker1(
      cortos::Thread::Entry([]{timed_worker("T1  ");}),
      timed1_stack,
      cortos::Thread::Priority(1));

   cortos::Thread timed_worker2(
      cortos::Thread::Entry([]{timed_worker("T2  ");}),
      timed2_stack,
      cortos::Thread::Priority(1));
   ...
   cortos::Thread blocking_thread(
      cortos::Thread::Entry([]{blocking_worker("BLOCK");}),
      blocking_stack,
      cortos::Thread::Priority(2));

   // Monitor at low priority (10)
   cortos::Thread monitor_thread(
      cortos::Thread::Entry([]{monitor_worker("MON");}),
      monitor_stack,
      cortos::Thread::Priority(10));
